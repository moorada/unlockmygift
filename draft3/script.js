...
const i18nDict = {
  ...
  es: {
    ...
    getYourGiftBtn: "Obtén Tu Regalo",
  ...
  fr: {
    ...
    getYourGiftBtn: "Obtenir Votre Cadeau",
  ...
  it: {
    ...
    getYourGiftBtn: "Ottieni il Tuo Regalo",
...
function initializeUnlocker() {
  ...
  // Cerca parametro GET lockedGift
  const urlParams = new URLSearchParams(window.location.search);
  const lockedGiftUrl = urlParams.get('lockedGift');

  // Se esiste un parametro lockedGift, carica direttamente il file
  if (lockedGiftUrl) {
    // Mostra un messaggio di caricamento
    if (uploadAreaEl) {
      uploadAreaEl.innerHTML = '<div class="loading-message">Loading gift from URL...</div>';
    }

    // Carica il file dall'URL specificato
    fetch(lockedGiftUrl)
      .then(response => {
        if (!response.ok) {
      ...
      .then(async fileContent => {
        try {
          ...
          // Passa direttamente alla visualizzazione delle informazioni del quiz
          goToQuizInfo();
        } catch (error) {
          ...
          if (uploadAreaEl) {
            uploadAreaEl.innerHTML = `
              <div class="error-message">
                Error loading gift: ${error.message}
                <br><br>
                <button class="btn" id="retryManualUploadBtn">Try manual upload</button>
              </div>
            `;

            const retryBtn = document.getElementById('retryManualUploadBtn');
            if (retryBtn) {
              retryBtn.addEventListener('click', () => {
                window.location.href = window.location.pathname; // Ricarica la pagina senza parametri
              });
            }
          }
        }
    ...
    // Inizializzazione di FilePond per upload manuale
    let pond;
  ...
  async function showCurrentQuestion() {
    ...
    submitBtn.addEventListener('click', async () => {
      ...
      if (correct) {
        ...
        // Determina quale bottone mostrare in base alla posizione
        let buttonHtml;
        if (currentQuestionIndex >= quizData.questions.length - 1) {
          // Ultimo indovinello, mostra "Ottieni il tuo regalo"
          buttonHtml = `<button class="btn" id="nextRiddleBtn">${dict.getYourGiftBtn || "Get Your Gift"}</button>`;
        } else {
          // Indovinelli intermedi, mostra solo continua
          buttonHtml = `<button class="btn" id="nextRiddleBtn">${dict.nextRiddleBtn}</button>`;
        }

        // Mostra messaggio di successo con un solo pulsante
        quizContainerEl.innerHTML = `
